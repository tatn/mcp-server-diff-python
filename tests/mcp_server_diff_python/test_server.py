import pytest
from mcp_server_diff_python.server import handle_list_tools, handle_call_tool
...
async def test_handle_list_tools():
    ...
    assert tool.description == "Get the difference between two text articles in Unified diff format. Use this when you want to extract the difference between texts."
...
async def test_handle_call_tool_missing_string_a():
    with pytest.raises(ValueError, match="Missing 'string_a' or 'string_b' in arguments"):
